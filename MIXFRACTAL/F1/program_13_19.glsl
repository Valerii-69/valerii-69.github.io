...
float fractal3D(vec3 p){
    vec3 q = fract(p * 2.0) * 2.0 - 1.0;
    float d = 1e9;
    for(int i=0; i<3; i++){
        vec3 offset = vec3(float(i)*0.3, float(i)*0.4, float(i)*0.2);
        vec3 crystal = abs(q + offset) - vec3(0.1, 0.2, 0.15);
        d = min(d, length(max(crystal, 0.0)) + min(max(crystal.x, max(crystal.y, crystal.z)), 0.0));
    }
    return d * 0.3;
}
