...
float x = uv.x;
float y = uv.y;
float a = 1.7 + bass*0.3;
float b = 1.3 + midBand*0.3;
float c = 0.7 + treble*0.3;
float d = 1.2;
for(int i=0;i<10;i++){
float nx = sin(ay) + ccos(ax);
float ny = sin(bx) + dcos(by);
x = nx; y = ny;
}
float v = length(vec2(x,y));
v = clamp(v, 0.0, 1.0);
return vec3(u_fon)*(v*bass, v*midBand, v*treble);
}
...
float fractal3D(vec3 p){
    float helixAngle = p.z * 4.0;
    vec2 helix1 = vec2(cos(helixAngle), sin(helixAngle)) * 0.3;
    vec2 helix2 = vec2(cos(helixAngle + 3.14159), sin(helixAngle + 3.14159)) * 0.3;
    
    float strand1 = length(p.xy - helix1) - 0.05;
    float strand2 = length(p.xy - helix2) - 0.05;
    
    // Connecting bars
    float t = mod(helixAngle, 3.14159*2.0);
    float bars = length(vec3(p.xy - mix(helix1, helix2, smoothstep(0.0, 1.0, t/(3.14159*2.0))), p.z)) - 0.02;
    
    return min(strand1, min(strand2, bars));
}
