...
float fractal3D(vec3 p){
    ...
    float d = 1e9;
    for(int i=0; i<4; i++){
        p = abs(p) - vec3(0.8, 0.6, 0.7);
        p.xy = p.xy * mat2(0.866, -0.5, 0.5, 0.866);
        p = p * scale;
        float sphere = length(p) - 1.0;
        float box = length(max(abs(p) - vec3(0.4, 0.3, 0.5), 0.0)) - 0.05;
        d = min(d, min(sphere, box) / pow(scale, float(i)));
    }
    return d;
}
