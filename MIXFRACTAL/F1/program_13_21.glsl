...
float fractal3D(vec3 p){
    float t = u_time * 0.5;
    vec3 q = p;
    q.x += 0.5 * sin(t + p.z * 2.0);
    q.y += 0.5 * cos(t + p.x * 2.0);
    q.z += 0.5 * sin(t + p.y * 2.0);
    
    float field = 0.0;
    for(int i=0; i<3; i++){
        float fi = float(i);
        field += sin(q.x * (3.0 + fi) + t) * 
                 cos(q.y * (4.0 + fi) + t*1.3) * 
                 sin(q.z * (5.0 + fi) + t*0.7);
        q = q * 1.3 + vec3(0.1, 0.2, -0.1);
    }
    return length(p) - 1.0 + 0.2 * field;
}
