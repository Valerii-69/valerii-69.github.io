...
float fractal3D(vec3 p){
    float r = length(p);
    float angle = atan(length(p.xy), p.z);
    
    float flare = r - 1.0;
    flare += 0.3 * sin(angle * 12.0 + u_time) * exp(-r*2.0);
    flare += 0.2 * sin(angle * 8.0 + u_time*1.3) * exp(-r*3.0);
    flare += 0.1 * sin(angle * 20.0 + u_time*0.7) * exp(-r*4.0);
